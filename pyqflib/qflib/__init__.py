...
def ppolySum(bkpoints1, values1, bkpoints2, values2, porder):
    ...
    Returns
    -------
    tuple(2D numpy array, 1D numpy array)
        breakpoints and values of the sum
    
    Notes
    -----
    1. If `porder`==0, the value at the left breakpoint is used for the segment.
    2. The result is a piecewise constant function with breakpoints at all breakpoints in `bkpoints1` and `bkpoints2`.
       The values are computed as the sum of the two functions at each breakpoint.
    """
