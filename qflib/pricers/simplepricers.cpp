...
double gamma = ((-exp(-intRate * timeToExp)) / (pow(spot, 2) * pow(volatility, 2) * timeToExp)) * d1 * normal.pdf(d2);
double theta = -intRate * exp(-intRate * timeToExp) * normal.cdf(phi * d2) - phi * exp(-intRate * timeToExp) * normal.pdf(d2) * (d1 / (2 * timeToExp));
